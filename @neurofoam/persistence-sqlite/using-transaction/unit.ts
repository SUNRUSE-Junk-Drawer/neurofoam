  ...
  describe(`usingTransaction`, () => {
    describe(`when the file path is invalid`, () => {
      ...
      beforeAll(async () => {
        ...
        try {
          await usingTransaction(
            `file:${path.join(
              os.tmpdir(),
              `neurofoam-host-temp-g.sqlite`
            )}?vfs=invalid`,
            body
          );
        } catch (e) {
    ...
    describe(`when the body callback resolves`, () => {
      ...
      beforeAll(async () => {
        ...
        body = jasmine
          ...
          .and.callFake(async (database: sqlite.Database) => {
            ...
            await database.run(
              `INSERT INTO entries (value) VALUES ('Test Entry Value');`
            );
        ...
        await using(
          filename,
          async (database) =>
            await database.run(`CREATE TABLE entries ( value TEXT );`)
        );
      ...
      it(`did not roll back any changes made`, () =>
        expectAsync(
          using(filename, (database) =>
            database.get(`SELECT value FROM entries;`)
          )
        ).toBeResolvedTo({ value: `Test Entry Value` }));
    });
    ...
    describe(`when the body callback rejects`, () => {
      ...
      it(`rolled back any changes made`, () =>
        expectAsync(
          using(filename, (database) =>
            database.get(`SELECT value FROM entries;`)
          )
