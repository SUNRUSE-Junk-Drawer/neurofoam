...
export default class <
  ...
  async initialize(): Promise<void> {
    await using(
      this.filename,
      database => database.migrate({ migrationsPath: path.join(__dirname, `migrations`) }),
    )
  }
    ...
    return await usingTransaction(
      ...
      async database => {
        try {
          ...
          if (e.message === `SQLITE_CONSTRAINT: UNIQUE constraint failed: bubbles.bubbleUuid`) {
    ...
    return await usingTransaction(
      ...
      async database => {
        await database.run(
          ...
          [nextEventUuid, previousEventUuid, bubbleUuid, sessionUuid, JSON.stringify(event), +new Date]
        )

        const rows = (await database.run(
          `UPDATE bubbles SET currentStateJson = ?, latestEventUuid = ? WHERE bubbleUuid = ? AND latestEventUuid = ?;`,
          [JSON.stringify(resultingState), nextEventUuid, bubbleUuid, previousEventUuid],
        )).changes
