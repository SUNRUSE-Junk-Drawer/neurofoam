  ...
  describe(`index`, () => {
    ...
    describe(`when the database is used correctly`, () => {
      ...
      beforeAll(async () => {
        filename = path.join(
          os.tmpdir(),
          `neurofoam-persistence-sqlite-temp-a.sqlite`
        );
        ...
        instance = new NeurofoamPersistenceSqlite<TestState, TestEvent>(
          filename
        );
    ...
    describe(`when recovering from version 001`, () => {
      ...
      beforeAll(async () => {
        filename = path.join(
          os.tmpdir(),
          `neurofoam-persistence-sqlite-temp-b.sqlite`
        );
        ...
        await fs.promises.copyFile(
          path.join(__dirname, `recover-001.sqlite`),
          filename
        );
      ...
      it(`does not delete the database file`, () =>
        expectAsync(fs.promises.access(filename)).toBeResolved());
      it(`getBubble returns null for nonexistent bubbles`, () =>
        expectAsync(
          instance.getBubble(`Test Nonexistent Bubble Uuid`)
    ...
    describe(`when recording a first event fails unexpectedly`, () => {
      ...
      beforeAll(async () => {
        filename = path.join(
          os.tmpdir(),
          `neurofoam-persistence-sqlite-temp-c.sqlite`
        );
      ...
      it(`rethrows the error`, () =>
        expect(thrown).toEqual(
          new Error(`SQLITE_ERROR: no such table: bubbles`)
        ));
    });
  });
});
