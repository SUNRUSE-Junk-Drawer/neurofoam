...
const errorResponseStatusCodeMappings: { readonly [error in neurofoamTypes.ErrorResponse]?: number } = {
...
export default class implements neurofoamTypes.Channel<
  ...
  async getSessionUuid(
    ...
    if (instance.request.headers.authorization === undefined || instance.request.headers.authorization.trim() === ``) {
      return {
        type: `none`,
      }
    }

    const match = authorizationRegex.exec(instance.request.headers.authorization)
  ...
  async checkMetadata(
    instance: CheckMetadataInstance
  ): Promise<boolean> {
    if (instance.request.method !== `POST`) {
      await new Promise(resolve => instance.response.writeHead(405).end(resolve))
      return false
    }

    if (!isContentTypeHeaderValid(instance.request.headers[`content-type`])) {
      await new Promise(resolve => instance.response.writeHead(415).end(resolve))
      return false
    }

    if (!isContentEncodingHeaderValid(instance.request.headers[`content-encoding`])) {
      await new Promise(resolve => instance.response.writeHead(415).end(resolve))
      return false
    }

    if (!isAcceptHeaderValid(instance.request.headers.accept)) {
      await new Promise(resolve => instance.response.writeHead(406).end(resolve))
      return false
    }

    if (!isAcceptCharsetHeaderValid(instance.request.headers["accept-charset"])) {
      await new Promise(resolve => instance.response.writeHead(406).end(resolve))
      return false
    }

    if (!isAcceptEncodingHeaderValid(instance.request.headers["accept-encoding"])) {
      await new Promise(resolve => instance.response.writeHead(406).end(resolve))
      return false
    }
  ...
  async respond(
    ...
    await new Promise(resolve => {
      if (sessionUuidIfNew === null) {
        ...
        instance.response.writeHead(200, { Authorization: `BEARER ${sessionUuidIfNew}` })
      }
