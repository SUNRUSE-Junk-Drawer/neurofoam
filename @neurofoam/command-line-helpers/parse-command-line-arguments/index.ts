      ...
      for (const key in ofType) {
        ...
        if (parameter.name[nameKey] === name) {
          if (
            ...
            fail(
              `Command-line parameter "-${parameter.name.short}"/"--${parameter.name.long}" specified multiple times.`
            );
          }
  ...
  for (const type in namedParameters) {
    ...
    for (const key in ofType) {
      ...
      if (
        ...
        fail(
          `Command-line parameter "-${parameter.name.short}"/"--${parameter.name.long}" not specified.`
        );
      }
    }
  }
