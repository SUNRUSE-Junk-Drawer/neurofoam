...
describe(`@neurofoam/persistence-dynamodb`, () => {
  describe(`index`, () => {
    ...
    function dynamoDbSettings(
      port: number
    ): awsSdk.DynamoDB.ClientConfiguration {
      return {
    ...
    describe(`when the bubble table cannot be created`, () => {
      ...
      beforeAll(async () => {
        ...
        instance = new NeurofoamPersistenceDynamoDb<TestState, TestEvent>(
          dynamoDbSettings(61341),
          {
            tableName: `BT`,
            attributeNames: {
              bubbleUuid: `Test-Bubble-Table-Bubble-Uuid-Attribute-Name`,
      ...
      it(`rejects as expected`, () =>
        expectAsync(instance.initialize()).toBeRejectedWithError(
    ...
    describe(`when the bubble table exists but not the event table`, () => {
      ...
      beforeAll(async () => {
        ...
        const previousInstance = new NeurofoamPersistenceDynamoDb<
          TestState,
          TestEvent
        >(dynamoDbSettings(61343), bubbleTableSettings, {
          tableName: `ET`,
          attributeNames: {
            eventUuid: `Test-Event-Table-Event-Uuid-Attribute-Name`,
            previousEventUuid: `Test-Event-Table-Previous-Event-Uuid-Attribute-Name`,
      ...
      it(`does not modify the bubble table`, async () =>
        expect(
          await new awsSdk.DynamoDB(dynamoDbSettings(61343))
            .describeTable({ TableName: `Test-Bubble-Table-Name` })
            .promise()
        ).toEqual({
          Table: {
      ...
      it(`creates the event table as per settings`, async () =>
        expect(
          await new awsSdk.DynamoDB(dynamoDbSettings(61343))
            .describeTable({ TableName: `Test-Event-Table-Name` })
            .promise()
        ).toEqual({
          Table: {
            AttributeDefinitions: [
    ...
    describe(`when the database is used correctly`, () => {
      ...
      beforeAll(async () => {
        ...
        resultOfCreatingBubble = await instance.recordFirstEvent(
          newlyCreatedBubbleUuid,
          newlyCreatedEventUuidA,
          sessionUuidA,
        ...
        await instance.recordFirstEvent(
          updatedOnceBubbleUuid,
          updatedOnceEventUuidA,
          sessionUuidB,
        ...
        await instance.recordFirstEvent(
          updatedTwiceBubbleUuid,
          updatedTwiceEventUuidA,
          sessionUuidD,
        ...
        await instance.recordFirstEvent(
          twiceCreatedBubbleUuid,
          twiceCreatedEventUuidA,
          sessionUuidG,
        ...
        await instance.recordFirstEvent(
          updateConflictABubbleUuid,
          updateConflictAEventUuidA,
          sessionUuidI,
        ...
        await instance.recordFirstEvent(
          updateConflictBBubbleUuid,
          updateConflictBEventUuidA,
          sessionUuidL,
      ...
      it(`creates the bubble table as per settings`, async () =>
        expect(
          await new awsSdk.DynamoDB(dynamoDbSettings(61344))
            .describeTable({ TableName: `Test-Bubble-Table-Name` })
            .promise()
        ).toEqual({
          Table: {
      ...
      it(`getBubble returns null for nonexistent bubbles`, () =>
        expectAsync(instance.getBubble(nonexistentBubbleUuid)).toBeResolvedTo(
          null
        ));
      it(`getBubble returns details for newly created bubbles`, () =>
        expectAsync(instance.getBubble(newlyCreatedBubbleUuid)).toBeResolvedTo({
      ...
      it(`getBubble returns details for bubbles updated once`, () =>
        expectAsync(instance.getBubble(updatedOnceBubbleUuid)).toBeResolvedTo({
      ...
      it(`getBubble returns details for bubbles updated twice`, () =>
        expectAsync(instance.getBubble(updatedTwiceBubbleUuid)).toBeResolvedTo({
      ...
      it(`recordFirstEvent makes no change should the bubble already exist`, () =>
        expectAsync(instance.getBubble(twiceCreatedBubbleUuid)).toBeResolvedTo({
      ...
      it(`recordSubsequentEvent makes no changes should the bubble be updated once since the call to getBubble`, () =>
        expectAsync(
          instance.getBubble(updateConflictABubbleUuid)
        ).toBeResolvedTo({
      ...
      it(`recordSubsequentEvent makes no changes should the bubble be updated twice since the call to getBubble`, () =>
        expectAsync(
          instance.getBubble(updateConflictBBubbleUuid)
        ).toBeResolvedTo({
    ...
    describe(`when connecting to a previously created database `, () => {
      ...
      beforeAll(async () => {
        ...
        const previousInstance = new NeurofoamPersistenceDynamoDb<
          TestState,
          TestEvent
        >(dynamoDbSettings(61345), bubbleTableSettings, eventTableSettings);

        await previousInstance.initialize();

        await previousInstance.recordFirstEvent(
          newlyCreatedBubbleUuid,
          newlyCreatedEventUuidA,
          sessionUuidA,
          { eventValue: `Test Event Value A` },
        ...
        await previousInstance.recordSubsequentEvent(
          updatedOnceBubbleUuid,
          updatedOnceEventUuidA,
          updatedOnceEventUuidB,
          sessionUuidC,
      ...
      it(`does not modify the bubble table`, async () =>
        expect(
          await new awsSdk.DynamoDB(dynamoDbSettings(61345))
            .describeTable({ TableName: `Test-Bubble-Table-Name` })
            .promise()
        ).toEqual({
          Table: {
      ...
      it(`does not modify the event table`, async () =>
        expect(
          await new awsSdk.DynamoDB(dynamoDbSettings(61345))
            .describeTable({ TableName: `Test-Event-Table-Name` })
            .promise()
      ...
      it(`getBubble returns details for bubbles created twice`, () =>
        expectAsync(instance.getBubble(twiceCreatedBubbleUuid)).toBeResolvedTo({
          currentState: { stateValue: `Test State Value G` },
          latestEventUuid: twiceCreatedEventUuidA,
        }));
      it(`getBubble returns details for bubbles with conflicting updates`, () =>
        expectAsync(
          instance.getBubble(updateConflictABubbleUuid)
        ).toBeResolvedTo({
          currentState: { stateValue: `Test State Value J` },
      ...
      it(`getBubble returns details for bubbles with multiple conflicting updates`, () =>
        expectAsync(
          instance.getBubble(updateConflictBBubbleUuid)
        ).toBeResolvedTo({
          currentState: { stateValue: `Test State Value N` },
    ...
    describe(`when an unexpected error is encountered while recording a new bubble`, () => {
      ...
      beforeAll(async () => {
        ...
        await new NeurofoamPersistenceDynamoDb<TestState, TestEvent>(
          dynamoDbSettings(61346),
          {
      ...
      it(`rejects as expected`, () =>
        expectAsync(
          instance.recordFirstEvent(
            `61420c90-df36-4237-85ca-661fa15c9399`,
            `b7a08ba5-6fca-4c99-9864-34a7cb8997c2`,
            `2b43a6aa-1a22-40a0-9ac5-969f01729332`,
            { eventValue: `Test Failing Event Value` },
            { stateValue: `Test Failing State Value` }
          )
        ).toBeRejectedWith(
          jasmine.objectContaining({ code: `ResourceNotFoundException` })
        ));
    });
    ...
    describe(`when an unexpected error is encountered while recording an update to an existing bubble`, () => {
      ...
      it(`rejects as expected`, () =>
        expectAsync(
          instance.recordSubsequentEvent(
            `61420c90-df36-4237-85ca-661fa15c9399`,
            `b7a08ba5-6fca-4c99-9864-34a7cb8997c2`,
            `2b43a6aa-1a22-40a0-9ac5-969f01729332`,
            `031c6390-3ab8-424f-8f02-cfba2b0d31ca`,
            { eventValue: `Test Failing Event Value` },
            { stateValue: `Test Failing State Value` }
          )
        ).toBeRejectedWith(
