...
>(
  ...
  const next = await application.requestCallback(
    application.initialState,
    executableRequest.sessionUuid,
  ...
  if (next.event !== null) {
    ...
    if (bubble === null) {
      const state = application.applyEvent(
        application.initialState,
        next.event
      );
      persistenceResult = await persistence.recordFirstEvent(
        executableRequest.bubbleUuid,
        await generateUuid(),
        executableRequest.sessionUuid,
        next.event,
        state
      ...
      persistenceResult = await persistence.recordSubsequentEvent(
        executableRequest.bubbleUuid,
        bubble.latestEventUuid,
        await generateUuid(),
        executableRequest.sessionUuid,
        next.event,
        state
